...
export default async function handleLogin(request, env) {
    ...
    if (!email || !password) {
        return setCORSHeaders(new Response(JSON.stringify({ success: false, error: "Email and password are required" }), {
            status: 400,
    ...
    if (!result || !(await verifyPassword(password, result.password, result.salt))) {
        return setCORSHeaders(new Response(JSON.stringify({ success: false, error: "Invalid email or password" }), {
            status: 401,
            headers: { "Content-Type": "application/json" }
        }));
    }
    ...
    return setCORSHeaders(new Response(JSON.stringify({ success: true, token }), {
        headers: { "Content-Type": "application/json" },
    }));
}
